...
ttstr = (
    pytalk.instance.sdk.ttstr
)  # Убедитесь, что sdk здесь доступен или импортируйте правильно
...
async def should_notify_user(
    ...
    try:
        from bot.database.models import (
            NotificationSetting as NotificationSettingEnum,
        )

        if notification_pref == NotificationSettingEnum.NONE:
            return False
        if (
            event_type == NOTIFICATION_EVENT_JOIN
            and notification_pref == NotificationSettingEnum.JOIN_OFF
        ):
            return False
        if (
            event_type == NOTIFICATION_EVENT_LEAVE
            and notification_pref == NotificationSettingEnum.LEAVE_OFF
        ):
            return False
    except ImportError:
        logger.error(
        ...
        if str(notification_pref.value) == "none":
            return False  # Сравнение со строковым значением enum
        if (
            event_type == NOTIFICATION_EVENT_JOIN
            and str(notification_pref.value) == "join_off"
        ):
            return False
        if (
            event_type == NOTIFICATION_EVENT_LEAVE
            and str(notification_pref.value) == "leave_off"
        ):
            return False
...
def _generate_join_leave_markup(
    ...
    callback_data = (
        f"toggle_ignore_user:{tt_user_username}:{callback_safe_nickname}"
    )
    button_text = get_text(
        "TOGGLE_IGNORE_BUTTON_TEXT",
        lang_code,
        nickname=button_display_nickname,
    )
...
async def send_join_leave_notification_logic(
    ...
    user_nickname_val = (
        ttstr(tt_user.nickname)
        or ttstr(tt_user.username)
        or get_text("WHO_USER_UNKNOWN", "en")
    )
    ...
    if (
        app_config.get("GLOBAL_IGNORE_USERNAME")
    ...
    def text_generator_func(lang_code: str) -> str:
        key_str = (
            "JOIN_NOTIFICATION"
            if event_type == NOTIFICATION_EVENT_JOIN
            else "LEAVE_NOTIFICATION"
        )
        return get_text(
            key_str,
            lang_code,
            user_nickname=html.quote(user_nickname_val),
