...
async def send_long_tt_reply(
    ...
    while remaining_text_val:
        if (
            len(remaining_text_val.encode("utf-8", errors="ignore"))
            <= max_len_bytes
        ):
        ...
        last_safe_split_index_in_chunk = (
            -1
        )  # Index within the current_chunk_str being built
        last_safe_split_index_in_remaining = (
            -1
        )  # Corresponding index in remaining_text_val
        ...
        for i, char_code_val in enumerate(remaining_text_val):
            ...
            if (
                i == len(remaining_text_val) - 1
            ):  # Reached end of remaining_text_val
                parts_to_send_list.append(current_chunk_str)
...
async def forward_tt_message_to_telegram_admin(
    ...
    tt_instance_for_check: TeamTalkInstance
    | None = None,  # For silent notification check
):
    ...
    admin_language = (
        admin_settings.language if admin_settings else DEFAULT_LANGUAGE
    )
    ...
    sender_display_val = (
        sender_nickname
        or sender_username
        or get_text("WHO_USER_UNKNOWN", admin_language)
    )
...
async def _tt_reconnect():
    ...
    while True:
        try:
            ...
            if (
                tt_bot_module.current_tt_instance
                and tt_bot_module.current_tt_instance.connected
                and tt_bot_module.current_tt_instance.logged_in
            ):
...
async def _tt_rejoin_channel(tt_instance: TeamTalkInstance):
    ...
    while True:
        if (
            ...
            or not tt_bot_module.current_tt_instance.connected
            ...
            if (
                not tt_bot_module.current_tt_instance
            ):  # If instance is gone, ensure reconnect is scheduled
                tt_bot_module.login_complete_time = None
