...
from aiogram import (
    html,
)  # For quoting in messages if needed, though not directly used here
...
async def handle_tt_subscribe_command(
    ...
    try:
        ...
        bot_info_val = (
            await tg_bot_event.get_me()
        )  # Get bot's username for the link
        deeplink_url_val = (
            f"https://t.me/{bot_info_val.username}?start={token_val}"
        )

        reply_text_val = get_text(
            "TT_SUBSCRIBE_DEEPLINK_TEXT",
            bot_language,
            deeplink_url=deeplink_url_val,
        )
        tt_message.reply(reply_text_val)
        logger.info(
            f"Generated subscribe deeplink {token_val} for TT user {ttstr(tt_message.user.username)}"
        )
    except Exception as e:
        logger.error(
            f"Error processing TT /sub command for {ttstr(tt_message.user.username)}: {e}",
            exc_info=True,
        )
...
async def handle_tt_unsubscribe_command(
    tt_message: TeamTalkMessage, session: AsyncSession, bot_language: str
):
    try:
        ...
        tt_message.reply(reply_text_val)
        logger.info(
            f"Generated unsubscribe deeplink {token_val} for TT user {ttstr(tt_message.user.username)}"
        )
    except Exception as e:
        logger.error(
            f"Error processing TT /unsub command for {ttstr(tt_message.user.username)}: {e}",
            exc_info=True,
        )
...
async def handle_tt_add_admin_command(
    ...
    if (
        not app_config.get("ADMIN_USERNAME")
        or sender_username_val != app_config["ADMIN_USERNAME"]
    ):
        logger.warning(
            f"Unauthorized /add_admin attempt by TT user {sender_username_val}."
        )
    ...
    try:
        ...
        for telegram_id_str_val in telegram_ids_to_add_list:
            if telegram_id_str_val.isdigit():
                ...
                if await add_admin(session, telegram_id_val):  # crud.add_admin
                    added_count_val += 1
                    logger.info(
                        f"Admin TG ID {telegram_id_val} added by TT admin {sender_username_val}"
                    )
                ...
                errors_list.append(
                    get_text(
                        ...
                        bot_language,
                        telegram_id_str=telegram_id_str_val,
                    )
                )

        reply_parts_list = []
        if added_count_val > 0:
            reply_parts_list.append(
                get_text(
                    "TT_ADD_ADMIN_SUCCESS", bot_language, count=added_count_val
                )
            )
        if errors_list:
            reply_parts_list.append(
                get_text("TT_ADMIN_ERRORS_HEADER", bot_language)
                + "\n- ".join(errors_list)
            )
        ...
        final_reply = (
            ...
            if reply_parts_list
            else get_text("TT_ADMIN_NO_VALID_IDS", bot_language)
        )
        tt_message.reply(final_reply)

    except Exception as e:
        logger.error(
            f"Error processing TT /add_admin command from {sender_username_val}: {e}",
            exc_info=True,
        )
...
async def handle_tt_remove_admin_command(
    ...
    if (
        ...
        logger.warning(
            f"Unauthorized /remove_admin attempt by TT user {sender_username_val}."
        )
        tt_message.reply(get_text("TT_ADMIN_CMD_NO_PERMISSION", bot_language))
        return
    ...
    try:
        ...
        if len(parts_list) < 2:
            tt_message.reply(
                get_text("TT_REMOVE_ADMIN_PROMPT_IDS", bot_language)
            )
            return
        ...
        for telegram_id_str_val in telegram_ids_to_remove_list:
            if telegram_id_str_val.isdigit():
                telegram_id_val = int(telegram_id_str_val)
                if await remove_admin_db(
                    session, telegram_id_val
                ):  # crud.remove_admin_db
                    removed_count_val += 1
                    logger.info(
                        f"Admin TG ID {telegram_id_val} removed by TT admin {sender_username_val}"
                    )
        ...
        )  # Or a specific "no valid IDs to remove"
        tt_message.reply(final_reply)

    except Exception as e:
        logger.error(
            f"Error processing TT /remove_admin command from {sender_username_val}: {e}",
            exc_info=True,
        )
        tt_message.reply(get_text("TT_ADMIN_ERROR_PROCESSING", bot_language))
...
async def handle_tt_not_on_online_command(
    ...
    try:
        ...
        reply_text = get_text(
            ...
            bot_language,
            tt_username=sender_tt_username,
            deeplink_url=deeplink_url,
        )
        tt_message.reply(reply_text)
        logger.info(
            f"Generated 'not on online' confirmation deeplink {token} for TT user {sender_tt_username}"
        )
    except Exception as e:
        logger.error(
            f"Error processing TT /not on online command for {sender_tt_username}: {e}",
            exc_info=True,
        )
...
async def handle_tt_help_command(
    tt_message: TeamTalkMessage, bot_language: str
):
    help_text_val = get_text(
        "HELP_TEXT", bot_language
    )  # Get the full help text object
...
async def handle_tt_unknown_command(
    ...
    reply_text_val = get_text(
        "TT_UNKNOWN_COMMAND", bot_language
    )  # Or TT_UNKNOWN_COMMAND_TT
    tt_message.reply(reply_text_val)
    logger.warning(
        f"Received unknown TT command from {ttstr(tt_message.user.username)}: {tt_message.content[:100]}"
    )
