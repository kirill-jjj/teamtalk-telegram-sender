...
async def on_ready():
    ...
    try:
        tt_bot_module.login_complete_time = (
            None  # Reset before connection attempt
        )
        ...
        logger.error(
            f"Error initiating TeamTalk server connection in on_ready: {e}",
            exc_info=True,
        )
...
async def on_my_login(server: PytalkServer):
    ...
    try:
        ...
        logger.error(
            f"Error during on_my_login (joining channel/setting status): {e}",
            exc_info=True,
        )
        if (
            tt_instance_val
        ):  # If instance exists, try to rejoin channel if that part failed
...
async def on_my_connection_lost(server: PytalkServer):
    ...
    if (
        ...
        and tt_bot_module.current_tt_instance.server_info.host == host
    ):
...
async def on_my_kicked_from_channel(channel_obj: PytalkChannel):
    ...
    try:
        ...
        channel_name_val = (
            ttstr(channel_obj.name) if channel_obj.name else "Unknown Channel"
        )
        ...
        logger.error(
            f"Error handling on_my_kicked_from_channel (channel ID: {channel_id_for_log}): {e}",
            exc_info=True,
        )
...
async def on_message(message: TeamTalkMessage):
    ...
    ):  # Ensure it's a private text message (type 1)
        return
