...
async def set_telegram_commands(bot: Bot):
    commands = [
        ...
        BotCommand(
            command="notify_all",
            description="Enable all join/leave notifications",
        ),
        BotCommand(
            command="notify_join_off",
            description="Disable join notifications only",
        ),
        BotCommand(
            command="notify_leave_off",
            description="Disable leave notifications only",
        ),
        BotCommand(
            command="notify_none",
            description="Disable all join/leave notifications",
        ),
        # Mute settings
        BotCommand(
            command="mute",
            description="Mute user (e.g., /mute user <tt_username>)",
        ),
        BotCommand(
            command="unmute",
            description="Unmute user (e.g., /unmute user <tt_username>)",
        ),
        BotCommand(
            command="mute_all",
            description="Mute all by default (except allow-list)",
        ),
        BotCommand(
            command="unmute_all",
            description="Unmute all by default (except block-list)",
        ),
        # Not on Online (NOON) feature
        BotCommand(
            command="toggle_noon",
            description="Toggle 'Not on Online' silent notifications",
        ),
        BotCommand(
            command="my_noon_status",
            description="Check your 'Not on Online' status",
        ),
