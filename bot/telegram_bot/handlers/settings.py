...
async def cl_command_handler(
    ...
    if not new_lang_candidate or new_lang_candidate.lower() not in [
        DEFAULT_LANGUAGE,
        "ru",
    ]:  # Assuming 'en' and 'ru'
        await message.reply(
...
async def _set_notification_preference(
    ...
    current_language = (
        user_specific_settings.language
    )  # Get current lang for reply
...
async def notify_leave_off_cmd(
    message: Message,
    session: AsyncSession,
    user_specific_settings: UserSpecificSettings,
):
...
async def _set_mute_all_in_settings(
    ...
    if (
        not mute_all_status
    ):  # When disabling mute_all, clear the exception list
...
async def unmute_command_handler(
    ...
    if (
        ...
        if (
            ...
            not in user_specific_settings.muted_users_set
        ):  # Not an exception, add it
            await _update_mute_user_in_settings(
                session,
                telegram_id_val,
                username_to_unmute_val,
...
async def toggle_noon_command_handler(
    ...
    if (
        not user_specific_settings.teamtalk_username
        or not user_specific_settings.not_on_online_confirmed
    ):
...
async def my_noon_status_command_handler(
    ...
    if (
        ...
        status_key_en = (
            "NOON_STATUS_ENABLED_EN"
            if user_specific_settings.not_on_online_enabled
            else "NOON_STATUS_DISABLED_EN"
        )
        status_key_ru = (
            "NOON_STATUS_ENABLED_RU"
            if user_specific_settings.not_on_online_enabled
            else "NOON_STATUS_DISABLED_RU"
        )
        ...
        effective_status_text = (
            get_text(status_key_en_str, "en")
            if language == "en"
            else get_text(status_key_ru, "ru")
        )
        reply_text = get_text(
            NOON_STATUS_REPORT,
            language,
