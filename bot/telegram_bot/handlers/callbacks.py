...
async def process_user_action_selection(
    ...
    try:
        ...
        action_val, user_id_str_val, user_nickname_val = (
        ...
        if action_val in [CALLBACK_ACTION_KICK, CALLBACK_ACTION_BAN]:
            if not is_admin_caller:
                ...
                await callback_query.answer(
                    get_text("CALLBACK_NO_PERMISSION", language),
                    show_alert=True,
                )
...
async def process_toggle_ignore_user(
    ...
    is_currently_in_set = (
        tt_username_to_toggle_val in user_specific_settings.muted_users_set
    )

    if (
        user_specific_settings.mute_all_flag
    ):  # Mute all is ON (set is allow list)
        if (
            is_currently_in_set
    ...
    feedback_key_str = (
        ...
        else "TOGGLE_IGNORE_NO_LONGER_IGNORED"
    )
    feedback_msg_for_answer_val = get_text(
        feedback_key_str,
        language,
        nickname=html.quote(nickname_from_callback_val),
    )
