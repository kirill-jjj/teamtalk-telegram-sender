...
async def start_command_handler(
    ...
    if not message.from_user:
        return  # Should not happen with Command filter
...
def _get_user_display_channel_name(
    ...
    if channel_obj:
        try:
            ...
            if (
                ...
                and (
                    channel_obj.channel_type
                    & pytalk.instance.sdk.ChannelType.CHANNEL_HIDDEN
                )
                != 0
            ):
    ...
    if channel_obj and channel_obj.id not in [
        WHO_CHANNEL_ID_ROOT,
        WHO_CHANNEL_ID_SERVER_ROOT_ALT,
        WHO_CHANNEL_ID_SERVER_ROOT_ALT2,
    ]:  # Regular channel
    ...
    elif not channel_obj or channel_obj.id in [
        WHO_CHANNEL_ID_SERVER_ROOT_ALT,
        WHO_CHANNEL_ID_SERVER_ROOT_ALT2,
...
async def who_command_handler(
    ...
    for user_obj in all_users_list:
        ...
        if (
            user_obj.id == tt_instance.getMyUserID()
            and not is_caller_admin_val
        ):
            continue
    ...
    if language == "ru":
        ...
        users_word_total_val = (
            get_text("WHO_USERS_COUNT_SINGULAR", "en")
            if user_count_val == 1
            else get_text("WHO_USERS_COUNT_PLURAL_5_MORE", "en")
        )
    ...
    text_reply = get_text(
        ...
        users_word=users_word_total_val,
    )
