...
async def set_mute_mode(
    ...
    current_settings_for_keyboard: UserSettings | None = None

    async with uow:
        user_settings = await uow.users.get_by_id(callback_query.from_user.id)
        if not user_settings:
            logger.error(
                "Could not find user_settings for user %s in set_mute_mode",
        ...
        updated_user_settings = await user_settings_service.update_mute_mode(
            user_settings, new_mode, actor=Actor.USER, uow=uow
        )
        ...
        if not updated_user_settings:
            ...
            success_toast_text = _("Mute list mode set to {mode}.").format(
                mode=mode_text
            )
            await callback_query.answer(success_toast_text)
            current_settings_for_keyboard = updated_user_settings

    if not current_settings_for_keyboard:
        # This case should ideally not be reached if the logic inside `async with`
        # is correct, but it's a safe fallback.
        logger.error(
            "current_settings_for_keyboard is None after attempting to set mute mode "
            "for user %s",
            callback_query.from_user.id,
        )
        await callback_query.answer(_(MSG_GENERAL_ERROR), show_alert=True)
