import logging
from typing import Callable, List, Dict # Added List, Dict
...
async def cq_show_language_menu(
    ...
    # callback_data: SettingsCallback, # Not strictly needed if action is filtered
...
async def cq_set_language(
    ...
    selected_lang_info = next((lang for lang in services.available_languages if lang["code"] == new_lang_code_str), None)
    ...
    try:
        ...
        # services.user_settings_cache[managed_user_settings.telegram_id].language_code = original_lang_code_str # More robust
        await callback_query.answer(_("An error occurred. Please try again later."), show_alert=True)
