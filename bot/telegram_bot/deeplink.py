...
async def _handle_confirm_noon_deeplink(
    ...
    user_specific_settings.not_on_online_enabled = (
        True  # Enable by default on confirmation
    )
    ...
    return get_text(
        ...
        language,
        tt_username=html.quote(tt_username_from_payload),
    )


# Define a type for the handler functions
DeeplinkHandlerType = Callable[
    [AsyncSession, int, str, Any, UserSpecificSettings],
...
async def handle_deeplink_payload(
    ...
    if (
        deeplink_obj.expected_telegram_id
        and deeplink_obj.expected_telegram_id != telegram_id_val
    ):
    ...
    if handler:
        try:
            ...
            logger.error(
                f"Error executing deeplink handler for action '{deeplink_obj.action}', token {token}: {e}",
                exc_info=True,
            )
