        ...
        if user.id not in admin_ids_cache:
            ...
            if not translator:
                ...
                if services:
                    ...
                    logger.warning("AdminCheckMiddleware: Translator and Services not found in data. Using temporary default translator.")
            ...
            logger.warning(f"AdminCheckMiddleware: Unauthorized user {user.id} (Username: {user.username}) for unhandled event type {type(event).__name__}. Behavior for this event type is undefined.")
