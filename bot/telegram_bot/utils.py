...
async def send_telegram_message_individual(
    ...
    reply_tt_method: Callable
    | None = None,  # For sending feedback to TeamTalk user
) -> bool:
    send_silently = False
    ...
    if (
        ...
        and recipient_settings.teamtalk_username
        and tt_instance_for_check
    ):
        tt_username_to_check = recipient_settings.teamtalk_username
        try:
            is_tt_user_online = False
            if (
                tt_instance_for_check.connected
                and tt_instance_for_check.logged_in
            ):
    ...
    try:
        ...
        if (
            "bot was blocked by the user" in str(e).lower()
...
async def send_telegram_messages_to_list(
    ...
    for chat_id_val in chat_ids:
        ...
        language_val = (
            user_settings_val.language
            if user_settings_val
        ...
        if (
            reply_markup_generator
            and tt_user_username_for_markup
            and tt_user_nickname_for_markup
        ):
...
async def show_user_buttons(
    ...
    for user_obj in users_list:
        ...
        user_nickname_val = (
            ttstr(user_obj.nickname)
            or ttstr(user_obj.username)
        ...
        callback_nickname_val = (
            ttstr(user_obj.nickname) or ttstr(user_obj.username) or "unknown"
        )[:CALLBACK_NICKNAME_MAX_LENGTH]
