...
*   **Validate Your Work:** After making changes, always run the project's validation suite: `uv run ruff check --fix` `uv run mypy` `uv run pytest` `uv run ruff format`.
...
#### Brief Guide to Naming Functions

When naming functions, focus on what the function *does* (its purpose or outcome), rather than *how* it achieves it or what triggers it. Aim for clarity, conciseness, and semantic accuracy.

*   **Start with a strong verb:** The name should clearly indicate the action performed (e.g., `get_user`, `create_report`, `send_notification`).
*   **Be specific about the outcome:** If the function returns a value, the name should hint at what that value represents (e.g., `fetch_user_profile`, `calculate_total`).
*   **Avoid implementation details:** Don't include words that describe internal mechanisms or temporary states (e.g., avoid `handle_`, `process_`, `_generate_`, `_orchestrate_`). The context (e.g., being in a handler or service) often provides enough information.
*   **Eliminate superfluous words:** Remove words like `get_all_`, `do_`, `perform_`, `manage_`, `effective_`, `safe_` if the core verb already conveys the meaning.
*   **For event handlers:** Use `on_` prefix to clearly indicate that the function reacts to an event (e.g., `on_user_joined`, `on_message_received`).
*   **For boolean functions/variables:** Name them as questions that can be answered with true/false (e.g., `is_admin`, `has_permission`, `can_proceed`).
...
5.  **Commit:** After successfully implementing and validating a task (including the runtime verification), you **must** always proactively offer to create a commit for the changes.
    ...
    *   **Tooling & Process:**
        0.  **Review changes:** Before drafting the commit message, always start by gathering information using shell commands:
