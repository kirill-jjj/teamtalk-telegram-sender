...
3.  **Установите зависимости**: `uv sync` автоматически создаст виртуальное окружение в папке `.venv`, если оно не существует, и установит все необходимые пакеты. Для разработки вам также необходимо установить дополнительные зависимости:
    ...
    uv sync --all-extras
    ```
4.  **Сгенерируйте файлы локализации**: Проект использует систему локализации на основе gettext. Для управления файлами локализации используются следующие команды `uv run`:
    *   Для извлечения всех переводимых строк из кода в файл шаблона (`locales/messages.pot`):
        ```bash
        uv run i18n extract
        ```
    *   Для создания или обновления `.po` файлов для конкретных языков (например, для русского `ru`, расположенного в `locales/ru/LC_MESSAGES/messages.po`):
        ```bash
        uv run i18n update
        ```
        (Скрипт `manage_locales.py`, вызываемый через `i18n`, динамически обнаружит языки на основе подкаталогов в `locales/`. Если вы добавляете новый язык, например, 'de', сначала создайте `locales/de/LC_MESSAGES/`, а затем выполните эту команду.)
    *   Для компиляции `.po` файлов в бинарные `.mo` файлы, используемые ботом во время выполнения:
        ```bash
        uv run i18n compile
        ```
    (Обычно, после первоначальной настройки, разработчики запускают `uv run i18n extract` при добавлении нового текста, затем `uv run i18n update`, затем переводят строки и, наконец, `uv run i18n compile` для компиляции.)
...
Для управления миграциями используется команда `uv run migrate`, которая является оберткой над `alembic`.
...
*   **Применение миграций**:
    ...
    uv run migrate upgrade head
    ```
    Если вы используете другой файл конфигурации (например, `prod.env`), передайте его через аргумент `--config`:
    ```bash
...
*   **Создание новой ревизии (миграции)**:
    ...
    ```
    С указанием другого файла конфигурации:
    ```bash
    uv run migrate --config prod.env revision -m "краткое_описание_изменений" --autogenerate
    ```
