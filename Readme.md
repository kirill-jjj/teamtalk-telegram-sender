...
1.  **Обновление каталогов языков (.po файлы)**:
    Когда вы добавляете или изменяете любые строки в коде Python, предназначенные для пользователя и требующие перевода (т.е. строки, обернутые в `_()`), вам необходимо обновить файл шаблона сообщений (`locales/messages.pot`) и `.po` файлы для каждого поддерживаемого языка. Команда `uv run i18n update` (вызывающая `manage_locales.py`) динамически определяет языки, ища подкаталоги в `locales/`.
    ```bash
    uv run i18n update
    ```
    Эта команда сначала извлекает переводимые строки из кодовой базы (согласно конфигурации в `babel.cfg`) в `locales/messages.pot`, а затем объединяет новые строки из `messages.pot` в каждый файл `<код_языка>/LC_MESSAGES/messages.po`. Новые строки будут добавлены, а измененные строки будут помечены как "fuzzy" (неточные) для проверки. Если вы создали новый каталог для языка (например, `locales/de/LC_MESSAGES/`), эта команда также инициализирует его файл `messages.po` на основе шаблона.
