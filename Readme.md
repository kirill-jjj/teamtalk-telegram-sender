...
7.  **Запустите бота**:
    ...
    uv run sender.py
    ```
    Вы также можете указать конкретный файл конфигурации (вместо `.env` по умолчанию), передав его как аргумент:
    ```bash
...
Все команды Alembic должны выполняться в активированном виртуальном окружении (`source .venv/bin/activate` или `uv run <command>`).

*   **Создание новой ревизии (миграции)**:
    После изменения моделей в `bot/models.py`, Alembic может попытаться автоматически сгенерировать скрипт миграции.
    ```bash
    uv run alembic revision -m "краткое_описание_изменений" --autogenerate
    ```
    Эта команда создаст новый файл миграции в `alembic/versions/`. Важно **проверить** этот файл и при необходимости внести ручные корректировки, так как автогенерация не всегда идеальна, особенно для сложных изменений.

*   **Применение миграций**:
    Чтобы применить все ожидающие миграции к базе данных (т.е. обновить схему до последней версии):
    ```bash
    uv run alembic upgrade head
    ```
